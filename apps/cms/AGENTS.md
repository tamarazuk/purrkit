...
This repo starts from the Payload CMS Cloudflare template and will evolve into a **multi-tenant headless CMS** for cat rescue websites. A separate Website Generator will create rescue sites that connect directly to this CMS by default. Each generated website maps to its own tenant here. None of the multi-tenancy work is implemented yet.

### Planned Architecture (High-Level)

- **Tenancy model**: Shared database with row-level isolation per tenant.
- **Tenant resolution**: Start with path-based resolution (e.g., `/t/{slug}`), with a migration path to host-based resolution later.
- **RBAC**: `super-admin` (global), `tenant-admin`, `editor`/`member`.
- **Default behavior**: The admin UI and Local API should be tenant-scoped for non-super-admin users.
- **Docs**: The working plan lives in `multi-tenancy-plan.md` and should stay aligned with new implementation work.
