...
The frontend is scaffolded out in this example directory. You can view the code for rendering pages at `src/app/(app)/tenant-slugs/[tenant]/[...slug]/page.tsx` and `src/app/(app)/tenant-domains/[tenant]/[...slug]/page.tsx`. This is a starter template, you may need to adjust the app to better fit your needs.

## Demo routes

This template keeps demo routes for both slug-based and domain-based tenant selection so you can explore different approaches while building a headless setup.

- Slug-based demo routes live under `/tenant-slugs/:tenant/...`
- Domain-based demo routes live under `/tenant-domains/:tenant/...`

For a fully headless implementation, tenants can host their sites elsewhere and consume the Payload REST API without using these demo routes.
